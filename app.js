...
var configjson  = require('./public/activity/zapier/config.json');
var indexhtml;
fs.readFile('./public/activity/zapier/index.html', "utf-8", function(err, html) {
	...
	if (!process.env.ACTIVITY_DESCRIPTION) process.env.ACTIVITY_DESCRIPTION = 'This Activity will make Zapier.';
...
app.post('/activity/zapier/save', activity.save );
app.post('/activity/zapier/validate', activity.validate );
app.post('/activity/zapier/publish', activity.publish );

//setup middleware for Marketing Cloud API calls:
//app.post('/activity/zapier/execute/', tokenFromJWT, activity.execute );
app.post('/activity/zapier/execute', activity.execute );

//replace template values with environment variables.
app.get( '/activity/zapier/config.json', function( req, res ) {
...
//replace template values with environment variables.
app.get( '/activity/zapier/index.html', function( req, res ) {
	res.status(200).send( indexhtml );		
});
app.get( '/activity/zapier/', function( req, res ) {
	res.status(200).send( indexhtml );		
});


// Custom Hello World Trigger Route
app.post('/activity/zapier/', trigger.edit );
